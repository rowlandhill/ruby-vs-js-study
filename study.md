...
```md
Ruby has class-based inheritance whereas JavaScript has prototypal inheritance.

Ruby has a single (strict) equality operator, `==`, whereas JavaScript has two
equality operators; a non-strict equality operator, `==`, that attempts to
perform type conversion, and a strict equality operator, `===`, that does not.

Ruby does not have unary decrement and increment operators (`--` and `++`).
```
